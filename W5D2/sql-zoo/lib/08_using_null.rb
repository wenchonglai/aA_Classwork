...
def teachers_and_mobiles
  ...
  execute(<<-SQL)
    SELECT
      name, COALESCE(mobile, '07986 444 2266') AS mobile_num
    FROM
      teachers
  SQL
end
...
def teachers_and_divisions
  ...
  execute(<<-SQL)
    SELECT
      teachers.name,
      CASE
        WHEN depts.name = 'Art' OR depts.id BETWEEN 1 AND 2
          THEN 'Sci'
        ELSE 'Art'
      END AS name
    FROM
      teachers LEFT JOIN depts ON depts.id = teachers.dept_id
  SQL
end
...
def teachers_and_divisions_two
  ...
  execute(<<-SQL)
    SELECT
      teachers.name,
      CASE
        WHEN depts.id BETWEEN 1 AND 2 THEN 'Sci'
